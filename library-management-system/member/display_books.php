...
<body>
	<div class="wrapper">
		...
		<div id="content">
			<h3>Book details</h3>
			<?php
            $query = $con->prepare("SELECT AUTH_ID, PUBLISHER_ID FROM book WHERE TITLE = ?");
            ...
            if ($rows == 0)
				...
				$author = $row["AUTH_ID"];
				$query = $con->prepare("SELECT * FROM book LEFT JOIN author ON author.AUTH_ID = book.AUTH_ID
				WHERE book.AUTH_ID = ?");
				$query->bind_param("s",$author);
				$query->execute();
				$resultn = $query->get_result();
