	...
	<div class="wrapper">
        ...
        if (isset($_POST['submit'])) {
	        ...
	        if (mysqli_num_rows($query->get_result()) != 1)
		        echo error_with_field("Invalid ISBN", "isbn");
	        else {
		        $query = $con->prepare("UPDATE book SET QUANTITY = QUANTITY + ? WHERE ISBN = ?;");
		        $query->bind_param("ds", $_POST['Quantity'], $_POST['isbn']);
		        if (!$query->execute())
			        die(error_without_field("ERROR: Couldn\'t update book copies"));
	        }
        }
        ?>
			<h1>Number of book copies has been updated!</h1>
		</div>
	</div>
</body>
