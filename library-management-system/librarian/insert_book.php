...
<body>
	<div class="wrapper">
	    ...
	    $query->bind_param("s", $_POST['isbn']);
	    $query->execute();

	    if (mysqli_num_rows($query->get_result()) != 0)
		    echo error_with_field("A book with that ISBN already exists", "isbn");
	    else {
		    $query = $con->prepare("INSERT INTO book VALUES(?, ?, ?, ?, ?, ?);");
		    $query->bind_param("ssssdd", $_POST['isbn'], $_POST['title'], $_POST['author'], $_POST['publisher'], $_POST['price'], $_POST['Quantity']);

		    if (!$query->execute())
			    echo "ERROR: Couldn't add book";
		    echo "New book details have been added in the database";
	    }
    }
    ?>
